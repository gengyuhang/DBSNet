...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (target, input_noisy, input_GT, std) in enumerate(self.data_loader):
            ...
            # print(target)
            # {'dir_idx': ['650', '610', '99', '522', '736', '270', '898', '1075', '969', '418', '173', '1025', '826', '833', '339', '1106']}
            #target_w,target_b,target_clean = self.model(input_GT) #z
            # newloss = self.loss(input_noisy,target_w)*0.5+self.loss(noise_w,target_w)*0.5+self.loss(input_GT,noise_w)*0.5 #z
            noisy,clean=self.model(input_noisy)
            target_noisy,target_clean=self.model(input_GT)
            
            noisy1,clean1=self.model(padr_crop((clean)))
            noisy2,clean2=self.model(padr_crop((noisy)))
            
            
                        
            loss1=self.loss(target_clean,clean1)
            loss2=self.loss(noisy2,target_noisy)
            # input_noisy_pred=noisy+clean
            # loss_con=self.loss(input_noisy_pred,input_noisy)
#            newloss = self.loss(input_noisy,target_clean)*0.5+self.loss(input_GT,clean)*0.5+self.loss(clean,target_clean) #z
            newloss=self.loss(input_noisy,target_clean)*0.5+self.loss(input_GT,clean)*0.5
...
#             loss_total = loss+.1*(loss_neg1+loss_neg2+loss_neg3+loss_neg4+loss_neg5+loss_neg6+loss_neg7)+newloss*0.1  #z
#            loss_total=newloss*0.5+loss1+loss2
            loss_total=newloss*0.5+loss1*0.25+loss2*0.25
            loss_total.backward()
